...
subtitle: Mamba is a novel architecture thast addresses Transformers’ computational inefficiency on long sequences/
...
Recently, *structured state space sequence models* (SSMs) were proposed as a new class of architectures for sequence modeling. These models can be interpreted as a combination of *RNN*s and *CNN*s, with inspirtaion from *classical state space models*. 
...
Therefore, the authors purpose a new class of SSM to achieve the performance of Transformers while scaling linearly.
...
### Architecture
The authors simplify the prior SSM architectures by combining the prior SSM architecutres with the MLP block of Transformers into a single block (Mamba).
...
## State Space Models
Structured state space models (S4) is a recent class of sequence moels for deep learning that are related to RNN, CNN, and classical state space models. They are defined with four parameters {% katexmm %}$(\Delta, A, B, C)${% endkatexmm %}and are inspired by a continuous system
...
A key property of equations (1), (2), and (3) is **Linear Time Invariance (LTI)**. This means that {% katexmm %}$(\Delta, A, B, C)${% endkatexmm %} and {% katexmm %}$(\bar{A}, \bar{B})${% endkatexmm %} are fixed for all time-steps. However, LTI models have foundamental limitations in modeling in certain types of data. Thus, the authors propose a method involve *removing* the LIT constraints while overcoming the efficiency bottlenecks.
...
The main idea is to only materialize the state {% katexmm %}$h${% endkatexmm %} only in more efficient levels of *memory hierarchy* by levearing properties of modern accelerators. In particular, most operations are bounded by memory bandwith, including our **scan operation**. We use *kernel fusion* to reduce the amount of memory IOs, significantly speed up the process implementation.
...
- Employ *recomputation* to reduce the memory requiremtns by avoiding saving the intermediate states.

### A Simplified SSM Architecture

Similar to other SSMs, selective SSMs are standalone *sequence transformations* that can be flexibly incorporated into neural networks. Previously, the most well-known SSM architecutre is the H3 architecture, which is comprised of a block inspired by *linear attention* interleaved with an *MLP* block. This work simplifies the above architecture by combining those two components into one.

The architecture expands the model dimension {% katexmm %}$D${% endkatexmm %} by a controllable expansion factor {% katexmm %}$E${% endkatexmm %}. Most of the parameters ({% katexmm %}$3ED^2${% endkatexmm %}) are in the linear projections, while the number of SSM parameters are much less in comparison. 
...
***Theorem 1.*** *When {% katexmm %}$N=1, A=-1, B=1, s_\Delta = \text{Linear}(x), \tau_\Delta = \text{softplus}$, then the selective SSM recurrence takes the form*
...
- A large {% katexmm %}$\Delta${% endkatexmm %} results the state {% katexmm %}$h${% endkatexmm %} and focuses on current input {% katexmm %}$x$. 
...
Modifying {% katexmm %}$B${% endkatexmm %} and {% katexmm %}$C${% endkatexmm %} allows **finer-grained** control over whether to let an input {% katexmm %}$x_t${% endkatexmm %} into the state {% katexmm %}$h_t${% endkatexmm %} or the state {% katexmm %}$h_t${% endkatexmm %} into the output {% katexmm %}$y_t${% endkatexmm %}. These can be interepted as allowing the model to modulate the recurrent dynamics based on the content (input) and context (hidden states) respectively
